"""Exact-snapshot H4 static Python import dependency graph intelligence."""
...
H4_GRAPH_POLICY_VERSION: str = "h4-v1"
DEPENDENCY_GRAPH_SCHEMA_VERSION: str = "1"

MAX_H4_SELECTED_ITEMS: int = 32
MAX_H4_IMPORT_EDGES_PER_FILE: int = 128
MAX_H4_TOTAL_IMPORT_EDGES: int = 1024
MAX_H4_MODULE_EXPRESSION_LENGTH: int = 256
MAX_H4_IMPORTED_NAME_LENGTH: int = 128
MAX_H4_RELATIVE_LEVEL: int = 64
MAX_H4_TOTAL_BODY_BYTES: int = 4_194_304


class RepositoryDependencyGraphError(HarnessError):
    """Base error for deterministic H4 dependency-graph failures."""


class RepositoryDependencyGraphBoundError(RepositoryDependencyGraphError):
    """Raised when an H4 hard resource bound would be exceeded."""


class RepositoryDependencyGraphConsistencyError(RepositoryDependencyGraphError):
    """Raised when exact H2/H3 content assertions contradict H4 re-analysis."""
...
def _edge_payload(
    *,
    ...
    return {
        ...
        "policy_version": H4_GRAPH_POLICY_VERSION,
...
class RepositoryImportDependency:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.relative_level) is not int or not (
            0 <= self.relative_level <= MAX_H4_RELATIVE_LEVEL
        ):
            raise HarnessValidationError(
                "relative_level must be an exact integer between 0 and "
                f"{MAX_H4_RELATIVE_LEVEL}: got {self.relative_level!r}"
            )
    ...
    def create(
        ...
        *,
        ...
        """Create one edge with its canonical H4 fingerprint."""
...
class RepositoryDependencyGraphResult:
    """Immutable H4 graph bound to exact H2, H3, snapshot, and source summaries."""
    ...
    policy_version: str = H4_GRAPH_POLICY_VERSION
    ...
    def __post_init__(self) -> None:
        ...
        if self.policy_version != H4_GRAPH_POLICY_VERSION:
            raise HarnessValidationError(
                f"policy_version must be {H4_GRAPH_POLICY_VERSION!r}: "
        ...
        if len(self.source_summary_fingerprints) > MAX_H4_SELECTED_ITEMS:
            raise RepositoryDependencyGraphBoundError(
                "source summary fingerprint count exceeds H4 selected-item hard limit "
                f"({MAX_H4_SELECTED_ITEMS})"
            )
        ...
        if len(self.edges) > MAX_H4_TOTAL_IMPORT_EDGES:
            raise RepositoryDependencyGraphBoundError(
                f"total import edge count exceeds hard limit ({MAX_H4_TOTAL_IMPORT_EDGES})"
            )
        ...
        def validate_current_group() -> None:
            if len(current_edges) > MAX_H4_IMPORT_EDGES_PER_FILE:
                raise RepositoryDependencyGraphBoundError(
                    "per-file import edge count exceeds hard limit "
                    f"({MAX_H4_IMPORT_EDGES_PER_FILE})"
                )
            if current_edges != sorted(current_edges, key=_edge_order_key):
                raise HarnessValidationError(
                    "dependency edges within one source must use deterministic H4 ordering"
                )
    ...
    def create(
        ...
        for edge in edges:
            ...
            if actual_resolution != expected_resolution:
                raise HarnessValidationError(
                    "dependency edge resolution does not match exact H4 alias policy"
                )
...
def _revalidate_upstream_bindings(
    ...
    if len(ranking.plan.selected_evidence) > MAX_H4_SELECTED_ITEMS:
        raise RepositoryDependencyGraphBoundError(
            f"H2 selected evidence count exceeds H4 hard limit ({MAX_H4_SELECTED_ITEMS})"
        )
...
def _extract_import_records(tree: ast.Module) -> tuple[_ImportRecord, ...]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            if node.level > MAX_H4_RELATIVE_LEVEL:
                raise RepositoryDependencyGraphBoundError(
                    f"relative import level exceeds hard limit ({MAX_H4_RELATIVE_LEVEL})"
                )
            _validate_bounded_text(
                module_expression,
                "module_expression",
        ...
        if len(records) > MAX_H4_IMPORT_EDGES_PER_FILE:
            raise RepositoryDependencyGraphBoundError(
                "per-file import edge count exceeds hard limit "
                f"({MAX_H4_IMPORT_EDGES_PER_FILE})"
            )
...
def _read_parsed_python_tree(
    ...
    if blob_size_bytes != summary.blob_size_bytes:
        raise RepositoryDependencyGraphConsistencyError(
            "H3 PARSED blob size contradicts exact H4 Git preflight size"
        )
    if blob_size_bytes > roles_module.MAX_H3_BLOB_BYTES:
        raise RepositoryDependencyGraphConsistencyError(
            "H3 PARSED blob exceeds the locked H3 per-blob body bound"
        )
    if aggregate_body_bytes + blob_size_bytes > MAX_H4_TOTAL_BODY_BYTES:
        raise RepositoryDependencyGraphBoundError(
            f"H4 aggregate body bytes exceed hard limit ({MAX_H4_TOTAL_BODY_BYTES})"
        )
...
def build_repository_dependency_graph(
    ...
    for evidence, summary in zip(ranking.plan.selected_evidence, roles.summaries):
        ...
        if len(edges) + len(records) > MAX_H4_TOTAL_IMPORT_EDGES:
            raise RepositoryDependencyGraphBoundError(
                f"total import edge count exceeds hard limit ({MAX_H4_TOTAL_IMPORT_EDGES})"
            )
        ...
        canonical_json_bytes(
            {
                "h2_plan_fingerprint": ranking.plan.plan_fingerprint,
                "h3_role_summary_fingerprint": roles.role_summary_fingerprint,
                "operation": "repository_dependency_graph",
                "policy_version": H4_GRAPH_POLICY_VERSION,
                "ranking_fingerprint": ranking.ranking_fingerprint,
                "schema_version": DEPENDENCY_GRAPH_SCHEMA_VERSION,
    ...
    receipt = HarnessReceipt(
        ...
        generator_version=H4_GRAPH_POLICY_VERSION,
