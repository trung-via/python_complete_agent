...
TASK-083 must accomplish four bounded outcomes:

```text
1. establish one shared validation-tier / owner contract;
2. introduce a bounded machine-readable ValidationPlan;
3. remove duplicate T2 full-canonical execution where ownership is already explicit;
4. record bounded evidence proving actual versus expected validation execution counts.
```
