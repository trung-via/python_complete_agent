...
The direct caller of `answer_grounded_context` must supply an already-built `CanonicalRagContext`. Question-to-retrieval-query planning is established in Phase 6 M4.5 (`plan_grounded_retrieval_query` in TASK-134), and persistent-knowledge application startup across the full catalog and explicit source manifests is established in Phase 6 M4.6 (`answer_persisted_grounded_question` in TASK-135). TASK-133 performs no retrieval, context construction, prompt reconstruction, JSON parsing, provider-specific behavior, storage, or product-truth reconciliation.
