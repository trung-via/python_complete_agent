...
def cmd_approve(args):
    fail(
        "Legacy 'approve' is disabled because it cannot create execution authority. "
        "Use 'bridge.py handoff <task_id> run|fix' through the canonical "
        "$aios-worker or /aios-worker surface."
    )
...
def build_parser():
    ...
    s = sub.add_parser(
        "approve", help="Deprecated: always fails closed; use the canonical handoff path"
    )
    s.add_argument("task_id", type=int)
