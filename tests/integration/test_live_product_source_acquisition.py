...
async def test_live_product_source_acquisition(tmp_path: Path) -> None:
    ...
    try:
        ...
        except (DiscoveryBlockedError, DiscoveryNavigationError, DiscoveryError, OrchestrationError) as exc:
            if isinstance(exc.__cause__, BrowserError) or "connect" in str(exc).lower():
                pytest.fail("LIVE_CDP_UNAVAILABLE", pytrace=False)
            pytest.fail("LIVE_DISCOVERY_UNAVAILABLE", pytrace=False)
        except Exception as exc:
            if isinstance(exc.__cause__, BrowserError):
                pytest.fail("LIVE_CDP_UNAVAILABLE", pytrace=False)
            pytest.fail("LIVE_DISCOVERY_UNAVAILABLE", pytrace=False)
