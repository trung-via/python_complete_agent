...
P8_PUBLIC_PDP_DOM_DIAGNOSTIC_GEN4_FAILURE_RECONCILIATION_FILE = (
    REPO_ROOT
    / "docs"
...
P8_PUBLIC_PDP_DOM_DIAGNOSTIC_GEN5_RESULT_AND_V3_FILE = (
    REPO_ROOT
    / "docs"
    / "PHASE_8_PUBLIC_TIKTOK_PDP_DOM_DIAGNOSTIC_GEN5_RESULT_RECONCILIATION_AND_V3_HARDENING.md"
)
P8_PUBLIC_PDP_DOM_DIAGNOSTIC_GEN6_AUTHORIZATION_FILE = (
    REPO_ROOT
    / "docs"
...
P8_PUBLIC_PDP_DOM_DIAGNOSTIC_GEN7_AUTHORIZATION_FILE = (
    REPO_ROOT
    / "docs"
    / "PHASE_8_PUBLIC_TIKTOK_PDP_DOM_DIAGNOSTIC_GEN7_AUTHORIZATION.md"
)
P8_PUBLIC_PDP_GEN7_RESULT_AND_PRICE_HARDENING_FILE = (
    REPO_ROOT
    / "docs"
    / "PHASE_8_PUBLIC_TIKTOK_PDP_GEN7_RESULT_AND_ROOT_SCOPED_PRICE_HARDENING.md"
)
P8_PUBLIC_TIKTOK_PDP_POST_TASK248_VALIDATION_CARRIER_HARDENING_FILE = (
    REPO_ROOT
    / "docs"
...
P8_PUBLIC_TIKTOK_PDP_POST_TASK250_VALIDATION_SUCCESS_RECONCILIATION_AND_PRICE_ROLE_OBSERVABILITY_FILE = (
    REPO_ROOT
    / "docs"
    / "PHASE_8_PUBLIC_TIKTOK_PDP_POST_TASK250_VALIDATION_SUCCESS_RECONCILIATION_AND_PRICE_ROLE_OBSERVABILITY.md"
)
P8_PUBLIC_PDP_DOM_DIAGNOSTIC_GEN8_AUTHORIZATION_FILE = (
    REPO_ROOT
    / "docs"
...
TASK_250_PUBLISHED_SOURCE_SHA = "3b4195b9f81e591697e94b7570450be069861552"
TASK_250_RUN_ID = "RUN-250-003"
TASK_250_REVIEW_ID = "REVIEW-250-001"
...
V2_ATTEMPT_MARKER_SHA256 = (
    "3588F3E2416B0E1BA8B3DEF49B73544B8C4D04BF3FEF01D17616393783F7C94E"
)
V2_RESULT_FILENAME = "tiktok-pdp-live-validation-result-v2.json"
V2_RESULT_SIZE_BYTES = 1947
V2_RESULT_SHA256 = (
    "808971B737F9C2647A7618827722E072A34BDEB07E8CD55325765B48D78CFAF2"
)
V5_DIAGNOSTIC_FILENAME = "tiktok-pdp-dom-diagnostic-v5.json"
...
def _historical_test_task_241_authorizes_one_exact_generation_4_v2_diagnostic_attempt():
    ...
    assert completed["TASK-241"]["post_publication_handoff"] == (
...
def test_task_241_history_preserves_its_handoff_without_freezing_current_global_handoff():
    state = load_yaml(ROADMAP_FILE)
    handoff = state["post_p8_planning_handoff"]
    authorization = handoff["public_pdp_dom_diagnostic_gen4_authorization"]
    completed = {item["task_id"]: item for item in state["completed_milestones"]}

    for record in (authorization, completed["TASK-241"]):
        assert record["source_task_id"] == "TASK-240"
        assert record["source_run_id"] == "RUN-240-002"
        assert record["source_review_id"] == "REVIEW-240-001"
        assert record["source_published_sha"] == TASK_240_PUBLISHED_SOURCE_SHA
...
def test_task_242_history_preserves_generation_4_failure_without_freezing_handoff():
    ...
    for record in (reconciliation, completed["TASK-242"]):
        ...
        assert record["source_published_sha"] == TASK_241_PUBLISHED_SOURCE_SHA
        assert record["diagnostic_v2_implementation_source_sha"] == (
            TASK_240_PUBLISHED_SOURCE_SHA
        )
    ...
    assert milestone["evaluation_failure_conclusions"] == {
        "selector": "NONE",
        "dom": "NONE",
        "render": "NONE",
        "shadow": "NONE",
    ...
    assert preflight["effective_only_after_separate_authorization"] is True
    assert preflight[
        "existing_task_137_borrowable_normal_page_set_must_unambiguously_resolve_to_exact_selected_pdp"
    ] is True
    ...
    for required in (
        ...
        "RUN-241-001",
        "REVIEW-241-001",
        TASK_241_PUBLISHED_SOURCE_SHA,
        TASK_240_PUBLISHED_SOURCE_SHA,
        "BOUNDED_CURRENT_PAGE_EVALUATION_FAILED",
        "generation_4_root_probe_observed=false",
...
def _historical_test_task_244_reconciles_generation_5_and_hardens_v3_without_authorizing_generation_6():
    state = load_yaml(ROADMAP_FILE)
    active = state["active_track"]
    milestone = active["current_milestone"]
    handoff = state["post_p8_planning_handoff"]
    authorization = handoff["generation_5_result_reconciliation_and_v3_hardening"]
    completed = {item["task_id"]: item for item in state["completed_milestones"]}
    document = P8_PUBLIC_PDP_DOM_DIAGNOSTIC_GEN5_RESULT_AND_V3_FILE.read_text(
        encoding="utf-8"
    )

    assert active["id"] == "P8_PUBLIC_PDP_DOM_DIAGNOSTIC_GEN5_RESULT_RECONCILIATION_AND_V3_HARDENING"
    assert active["sequence_status"] == "COMPLETE_ON_EXACT_TASK_244_SOURCE_PUBLICATION"
    assert milestone["task_id"] == "TASK-244"
    assert milestone["classification"] == "GENERATION_5_RESULT_RECONCILIATION_AND_TITLE_LOCAL_COMMERCE_OBSERVABILITY_V3_HARDENING_ONLY"
    for record in (milestone, authorization, completed["TASK-244"]):
        assert record["source_task_id"] == "TASK-243"
        ...
        assert record["generation_5_authorized_diagnostic_attempts_remaining"] == 0
        assert record["generation_5_diagnostic_execution_owner"] == "HUMAN_OPERATOR"
        assert record["generation_5_diagnostic_executed"] is True
        assert record["diagnostic_outcome"] == "FAIL_CLOSED"
    ...
    artifact = milestone["generation_5_artifact"]
    assert artifact == {"schema_version": 2, "filename": "tiktok-pdp-dom-diagnostic-v2.json", "sha256": "03CB57F0BA9838985233C919E144416416E1A17DD2DA033F8B5F8A3FE224EA1F", "size_bytes": 6130, "observed_at": "2026-09-22T23:00:39.526520+00:00", "evidence_authority": "NONE"}
    assert milestone["generation_5_root_probe"] == {"title_anchor_count": 1, "price_anchor_count": 0, "action_anchor_count": 0, "visible_explicit_pdp_root_count": 0, "explicit_root_with_commerce_anchors_count": 0, "main_present": False, "main_visible": False, "main_has_commerce_anchors": False, "multi_anchor_common_ancestor_found": False, "selected_root_kind": "NONE"}
    assert milestone["generation_5_commerce_probe"] == {"document_ready_state": "COMPLETE", "bounded_nodes_scanned": 600, "bounded_scan_truncated": True, "visible_currency_like_count": 0, "near_title_currency_like_count": 0, "visible_interactive_count": 106, "near_title_action_like_count": 91, "visible_loading_marker_count": 0, "open_shadow_root_count": 0, "visible_iframe_count": 0}
    assert milestone["selector_family_miss_scope"] == "FULL_DOCUMENT_BODY_CURRENT_SELECTOR_QUERIES_NOT_GLOBAL_SCAN_CAP"
    assert milestone["global_currency_negative_scope"] == "NON_EXHAUSTIVE_TRUNCATED_600_NODE_SCAN"
    assert milestone["legacy_near_title_action_count_scope"] == "BROAD_V2_ANCESTOR_CONTAINMENT_HEURISTIC_NOT_COMMERCE_CONTROL_EVIDENCE"
    assert milestone["diagnostic_schema_version"] == 3
    assert milestone["diagnostic_artifact_filename"] == "tiktok-pdp-dom-diagnostic-v3.json"
    assert milestone["generation_6_authorized"] is False
    ...
    for required in ("TASK-243", "RUN-243-001", "REVIEW-243-001", "1410e993de69a6c3a9f100d256328710976a9c64", "tiktok-pdp-dom-diagnostic-v2.json", "tiktok-pdp-dom-diagnostic-v3.json", "near-title action count of 91", "at most six", "at most 300", "HUMAN_BRAIN_FRESH_GENERATION_6_TITLE_LOCAL_COMMERCE_OBSERVABILITY_DIAGNOSTIC_AUTHORIZATION", "CAPABILITY_IS_NOT_AUTHORITY", "EVIDENCE_IS_NOT_PRODUCT_TRUTH", "SOURCE_IDENTITY_IS_NOT_CANONICAL_IDENTITY", "SNAPSHOT_IS_NOT_TREND", "ONE_CAPABILITY_ONE_AUTHORITY"):
        assert required in normalized_document


def test_task_244_history_preserves_reconciliation_without_freezing_current_global_handoff():
    state = load_yaml(ROADMAP_FILE)
    handoff = state["post_p8_planning_handoff"]
    reconciliation = handoff["generation_5_result_reconciliation_and_v3_hardening"]
    completed = {item["task_id"]: item for item in state["completed_milestones"]}
    document = P8_PUBLIC_PDP_DOM_DIAGNOSTIC_GEN5_RESULT_AND_V3_FILE.read_text(
        encoding="utf-8"
    )

    for record in (reconciliation, completed["TASK-244"]):
        assert record["source_task_id"] == "TASK-243"
        assert record["source_run_id"] == "RUN-243-001"
        assert record["source_review_id"] == "REVIEW-243-001"
        assert record["source_published_sha"] == "1410e993de69a6c3a9f100d256328710976a9c64"
        ...
        assert record["diagnostic_artifact_created"] is True
        assert record["live_dom_diagnostic_authority"] == "NONE"
        assert record["generation_6_authorized"] is False
        assert record["post_publication_handoff"] == (
            "HUMAN_BRAIN_FRESH_GENERATION_6_TITLE_LOCAL_COMMERCE_OBSERVABILITY_DIAGNOSTIC_AUTHORIZATION"
        )

    assert handoff["destination"] != reconciliation["post_publication_handoff"]
    assert reconciliation["title_local_topology_probe"] == {
        "max_ancestor_levels": 6,
        "max_descendant_elements_per_level": 300,
        "root_or_selector_authority": "NONE",
        "raw_marketplace_values_or_text_persisted": False,
        "shadow_root_traversal": False,
        "iframe_document_traversal": False,
    }
    normalized_document = " ".join(document.split())
    for required in (
        "TASK-243",
        "RUN-243-001",
        "REVIEW-243-001",
        "1410e993de69a6c3a9f100d256328710976a9c64",
        "tiktok-pdp-dom-diagnostic-v2.json",
        "tiktok-pdp-dom-diagnostic-v3.json",
        "near-title action count of 91",
        "at most six",
        "at most 300",
        "HUMAN_BRAIN_FRESH_GENERATION_6_TITLE_LOCAL_COMMERCE_OBSERVABILITY_DIAGNOSTIC_AUTHORIZATION",
        "CAPABILITY_IS_NOT_AUTHORITY",
...
def _historical_test_task_245_authorizes_one_exact_generation_6_v3_diagnostic_attempt():
    ...
    assert active["id"] == "P8_PUBLIC_PDP_DOM_DIAGNOSTIC_GEN6_AUTHORIZATION"
    assert active["sequence_status"] == "COMPLETE_ON_EXACT_TASK_245_SOURCE_PUBLICATION"
    assert milestone["task_id"] == "TASK-245"
    assert milestone["classification"] == (
    ...
    assert preflight["external_job_root_generation"] == 6
    ...
    assert preflight["fresh_external_job_root_requires_v3_absent"] is True
    assert preflight["execution_critical_file_equivalence_source_sha"] == (
        TASK_244_PUBLISHED_SOURCE_SHA
    )
    assert preflight["execution_critical_files"] == [
    ...
    assert milestone["root_probe_interpretation"] == "ENGINEERING_DIAGNOSTIC_HINT_ONLY"
    assert milestone["commerce_probe_interpretation"] == "ENGINEERING_DIAGNOSTIC_HINT_ONLY"
    assert milestone["title_local_topology_probe_interpretation"] == "ENGINEERING_DIAGNOSTIC_HINT_ONLY"
    assert milestone["truncated_scan_negative_observations"] == "NON_EXHAUSTIVE"
    assert milestone["shadow_or_iframe_traversal_authority"] == "NONE"
    assert milestone["readiness_or_selector_hint_repair_authority"] == "NONE"
    ...
    assert milestone["historical_generation_4"]["authorized_diagnostic_attempts_remaining"] == 0
    assert milestone["historical_generation_5"]["authorized_diagnostic_attempts_remaining"] == 0
    assert milestone["historical_generation_5"]["diagnostic_executed"] is True
    assert milestone["historical_generation_5"]["diagnostic_outcome"] == "FAIL_CLOSED"
    assert milestone["historical_generation_5"]["diagnostic_failure_reason"] == "NO_BOUNDED_PDP_ROOT"
    assert milestone["historical_generation_5"]["diagnostic_artifact_created"] is True
...
def test_task_246_history_preserves_reconciliation_and_v4_hardening_without_freezing_current_global_handoff():
    state = load_yaml(ROADMAP_FILE)
    handoff = state["post_p8_planning_handoff"]
...
def test_task_246_reconciles_generation_6_and_hardens_v4_root_without_authorizing_generation_7():
    test_task_246_history_preserves_reconciliation_and_v4_hardening_without_freezing_current_global_handoff()


def _historical_test_task_246_reconciles_generation_6_and_hardens_v4_root_without_authorizing_generation_7():
    state = load_yaml(ROADMAP_FILE)
    active = state["active_track"]
    milestone = active["current_milestone"]
    handoff = state["post_p8_planning_handoff"]
...
def test_task_247_authorizes_one_exact_generation_7_v4_diagnostic_attempt():
    test_task_247_history_preserves_authorization_without_freezing_current_global_handoff()


def test_task_247_history_preserves_authorization_without_freezing_current_global_handoff():
    state = load_yaml(ROADMAP_FILE)
    handoff = state["post_p8_planning_handoff"]
    authorization = handoff["public_pdp_dom_diagnostic_gen7_authorization"]
    completed = {item["task_id"]: item for item in state["completed_milestones"]}
    completed_task = completed["TASK-247"]
    document = P8_PUBLIC_PDP_DOM_DIAGNOSTIC_GEN7_AUTHORIZATION_FILE.read_text(
        encoding="utf-8"
    )

    assert authorization["classification"] == (
        "FRESH_GENERATION_7_BOUNDED_TITLE_LOCAL_COMMERCE_ROOT_DIAGNOSTIC_AUTHORIZATION_ONLY"
    )
    assert completed_task["classification"] == (
        "FRESH_GENERATION_7_BOUNDED_TITLE_LOCAL_COMMERCE_ROOT_DIAGNOSTIC_AUTHORIZATION_ONLY"
    )
    for record in (authorization, completed_task):
        assert record["source_task_id"] == "TASK-246"
        assert record["source_run_id"] == "RUN-246-002"
        assert record["source_review_id"] == "REVIEW-246-001"
        assert record["source_published_sha"] == TASK_246_PUBLISHED_SOURCE_SHA
        assert record["diagnostic_v4_implementation_source_sha"] == (
            TASK_246_PUBLISHED_SOURCE_SHA
        )
        assert record["generation_7_execution_source_sha"] == (
            TASK_246_PUBLISHED_SOURCE_SHA
        )
        assert record["diagnostic_authorization_generation"] == 7
        assert record["live_dom_diagnostic_authority"] in (
            "ONE_SHOT_ATTACH_ONLY_EXACT_LISTING",
            "NONE",
        )
    ...
    assert completed_task["post_publication_handoff"] == (
        "HUMAN_OPERATOR_GENERATION_7_BOUNDED_TITLE_LOCAL_COMMERCE_ROOT_DIAGNOSTIC_EXECUTION"
    )
    assert handoff["destination"] != completed_task["post_publication_handoff"]
    ...
    assert authorization["diagnostic_carrier"] == (
        "src/product_intelligence/tiktok_pdp_dom_diagnostic.py"
    )
    assert authorization["canonical_cli_command"] == "tiktok-pdp-dom-diagnostic"
    assert authorization["borrowed_session_count"] == 1
    assert authorization["session_evaluate_count"] == 1
    assert authorization["navigation_refresh_click_type_scroll_authority"] == "NONE"
    assert authorization["browser_lifecycle_authority"] == "TASK-137"
    assert authorization["task_137_cleanup_preserved"] is True
    ...
    assert preflight["sole_normal_page_is_exact_selected_pdp"] is True
    assert preflight["selected_pdp_source_id"] == SELECTED_SOURCE_ID
    assert preflight["external_job_root_generation"] == 7
    assert preflight["fresh_external_job_root_requires_v1_absent"] is True
    assert preflight["fresh_external_job_root_requires_v2_absent"] is True
    assert preflight["fresh_external_job_root_requires_v3_absent"] is True
    assert preflight["fresh_external_job_root_requires_v4_absent"] is True
    assert preflight["execution_critical_file_equivalence_source_sha"] == (
        TASK_246_PUBLISHED_SOURCE_SHA
    )
    assert preflight["execution_critical_files"] == [
    ...
    assert authorization["root_probe_interpretation"] == "ENGINEERING_DIAGNOSTIC_HINT_ONLY"
    assert authorization["commerce_probe_interpretation"] == "ENGINEERING_DIAGNOSTIC_HINT_ONLY"
    assert authorization["title_local_topology_probe_interpretation"] == "ENGINEERING_DIAGNOSTIC_HINT_ONLY"
    assert authorization["truncated_scan_negative_observations"] == "NON_EXHAUSTIVE"
    assert authorization["shadow_or_iframe_traversal_authority"] == "NONE"
    assert authorization["readiness_or_selector_hint_repair_authority"] == "NONE"
...
def test_task_248_reconciles_gen7_success_and_hardens_bounded_root_price_extraction():
    state = load_yaml(ROADMAP_FILE)
    handoff = state["post_p8_planning_handoff"]
    reconciliation = handoff["generation_7_result_reconciliation_and_root_scoped_price_hardening"]
    doc_entry = handoff["public_pdp_gen7_result_and_root_scoped_price_hardening"]
    completed = {item["task_id"]: item for item in state["completed_milestones"]}
    completed_task = completed["TASK-248"]
    document = P8_PUBLIC_PDP_GEN7_RESULT_AND_PRICE_HARDENING_FILE.read_text(
        encoding="utf-8"
    )

    assert completed_task["track_id"] == "P8_PUBLIC_PDP_GEN7_RESULT_AND_ROOT_SCOPED_PRICE_HARDENING"
    assert completed_task["task_id"] == "TASK-248"
    assert completed_task["classification"] == (
        "GENERATION_7_SUCCESS_RECONCILIATION_AND_BOUNDED_ROOT_SCOPED_PRICE_EXTRACTION_HARDENING_ONLY"
    )

    for record in (reconciliation, doc_entry, completed_task):
        assert record["source_task_id"] == "TASK-247"
        assert record["source_run_id"] == "RUN-247-002"
        assert record["source_review_id"] == "REVIEW-247-001"
    ...
    for record in (doc_entry, completed_task):
        assert record["diagnostic_authorization_generation"] == 7
    ...
    assert v4_artifact["filename"] == "tiktok-pdp-dom-diagnostic-v4.json"
    assert v4_artifact["sha256"] == GEN7_ARTIFACT_SHA256
    assert v4_artifact["size_bytes"] == GEN7_ARTIFACT_SIZE_BYTES
    assert v4_artifact["observed_at"] == "2026-09-23T18:42:34.938791+00:00"
    assert v4_artifact["evidence_authority"] == "NONE"

    root_probe = reconciliation["root_probe"]
    assert root_probe["title_anchor_count"] == 1
    assert root_probe["price_anchor_count"] == 0
    assert root_probe["action_anchor_count"] == 0
    assert root_probe["visible_explicit_pdp_root_count"] == 0
    assert root_probe["explicit_root_with_commerce_anchors_count"] == 0
    assert root_probe["main_present"] is False
    assert root_probe["main_visible"] is False
    assert root_probe["main_has_commerce_anchors"] is False
    assert root_probe["multi_anchor_common_ancestor_found"] is False
    assert root_probe["selected_root_kind"] == "TITLE_LOCAL_COMMERCE_QUORUM"
    assert root_probe["selected_title_local_ancestor_level"] == 2

    level_2 = reconciliation["selected_level_2_probe"]
    assert level_2["bounded_nodes_scanned"] == 90
    assert level_2["bounded_scan_truncated"] is False
    assert level_2["current_price_selector_match_count"] == 0
    assert level_2["current_action_selector_match_count"] == 0
    assert level_2["visible_currency_like_count"] == 3
    assert level_2["commerce_semantic_action_like_count"] == 2
    assert level_2["strong_commerce_action_like_count"] == 1
    assert level_2["paired_strong_commerce_control_count"] == 1
    assert level_2["title_local_root_quorum_satisfied"] is True

    for required in (
        "TASK-247",
        "RUN-247-002",
        "REVIEW-247-001",
        TASK_247_PUBLISHED_SOURCE_SHA,
        TASK_246_PUBLISHED_SOURCE_SHA,
        GEN7_ARTIFACT_SHA256,
        str(GEN7_ARTIFACT_SIZE_BYTES),
        "tiktok-pdp-dom-diagnostic-v4.json",
        "TITLE_LOCAL_COMMERCE_QUORUM",
        "HUMAN_BRAIN_LIVE_PUBLIC_PDP_PILOT_AUTHORIZATION",
...
def _historical_test_task_249_hardens_post_task248_validation_carrier_with_zero_live_authority():
    ...
    assert active["id"] == "P8_PUBLIC_PDP_POST_TASK248_VALIDATION_CARRIER_HARDENING"
    assert active["sequence_status"] == "COMPLETE_ON_EXACT_TASK_249_SOURCE_PUBLICATION"
    assert milestone["task_id"] == "TASK-249"
    assert milestone["classification"] == (
        "POST_TASK248_LIVE_PUBLIC_PDP_VALIDATION_CARRIER_HARDENING_ONLY"
    )

    for record in (milestone, carrier_entry, completed_task):
        assert record["source_task_id"] == "TASK-248"
        assert record["source_published_sha"] == TASK_248_PUBLISHED_SOURCE_SHA
        assert record["collector_validation_baseline"] == {
            "task_id": "TASK-248",
...
def test_task_250_authorizes_one_exact_post_task249_validation_attempt():
    test_task_250_history_preserves_authorization_without_freezing_current_global_handoff()


def test_task_250_history_preserves_authorization_without_freezing_current_global_handoff():
    state = load_yaml(ROADMAP_FILE)
    handoff = state["post_p8_planning_handoff"]
    ...
    assert authorization["live_public_pdp_acquisition_authority"] == "ONE_SHOT_EXACT_LISTING_ONLY"
    assert authorization["post_attempt_review_authority"] == (
        "HUMAN_BRAIN_POST_TASK249_PUBLIC_PDP_VALIDATION_REVIEW"
    )
    assert authorization["mandatory_post_validation_review"] == (
        "HUMAN_BRAIN_POST_TASK249_PUBLIC_PDP_VALIDATION_REVIEW"
    )
    assert handoff["destination"] != authorization["post_publication_handoff"]
...
def test_task_251_reconciles_post_task250_validation_success_and_hardens_v5_price_role_observability():
    test_task_251_history_preserves_reconciliation_without_freezing_current_global_handoff()


def test_task_251_history_preserves_reconciliation_without_freezing_current_global_handoff():
    state = load_yaml(ROADMAP_FILE)
    handoff = state["post_p8_planning_handoff"]
    reconciliation = handoff[
    ...
    completed = {item["task_id"]: item for item in state["completed_milestones"]}
    completed_task = completed["TASK-251"]
    document = (
        P8_PUBLIC_TIKTOK_PDP_POST_TASK250_VALIDATION_SUCCESS_RECONCILIATION_AND_PRICE_ROLE_OBSERVABILITY_FILE.read_text(
            encoding="utf-8"
        )
    )

    # AC1: Exact TASK-250 / RUN-250-003 / REVIEW-250-001 publication lineage at
    # 3b4195b9f81e591697e94b7570450be069861552 is reconciled while TASK-249 carrier source and
    # TASK-248 collector baseline remain separately identified and semantically distinct.
    for record in (reconciliation, completed_task):
        assert record["task_id"] == "TASK-251"
        assert record["classification"] == (
            "POST_TASK250_OPERATION_SUCCESS_RECONCILIATION_AND_BOUNDED_PRICE_ROLE_OBSERVABILITY_HARDENING_ONLY"
        )
        assert record["source_task_id"] == "TASK-250"
        assert record["source_run_id"] == TASK_250_RUN_ID
        assert record["source_review_id"] == TASK_250_REVIEW_ID
        assert record["source_published_sha"] == TASK_250_PUBLISHED_SOURCE_SHA
        assert record["validation_carrier_source_sha"] == TASK_249_PUBLISHED_SOURCE_SHA
        assert record["collector_validation_baseline"] == {
            "task_id": "TASK-248",
            "source_sha": TASK_248_PUBLISHED_SOURCE_SHA,
        ...
        # AC2: Human marketplace operation recorded exactly as one consumed SUCCESS / OBSERVED / SESSION_RELEASE_SUCCESS
        assert record["authorized_validation_attempts"] == 1
        assert record["authorized_validation_attempts_remaining"] == 0
        assert record["validation_execution_owner"] == "HUMAN_OPERATOR"
        ...
        assert record["process_exit_code"] == "UNKNOWN"

        # AC3: Exact external V2 marker and result artifact provenance
        assert record["validation_attempt_marker"]["filename"] == V2_ATTEMPT_MARKER_FILENAME
        assert record["validation_attempt_marker"]["schema_version"] == 2
        assert record["validation_attempt_marker"]["size_bytes"] == V2_ATTEMPT_MARKER_SIZE_BYTES
        assert record["validation_attempt_marker"]["sha256"] == V2_ATTEMPT_MARKER_SHA256

        assert record["validation_result"]["filename"] == V2_RESULT_FILENAME
        assert record["validation_result"]["schema_version"] == 2
        assert record["validation_result"]["size_bytes"] == V2_RESULT_SIZE_BYTES
        assert record["validation_result"]["sha256"] == V2_RESULT_SHA256

        # AC4: Explicit field uncertainty & OPERATION_SUCCESS_IS_NOT_FIELD_VALIDATION_SUCCESS
        uncertainty = record["field_validation_uncertainty"]
        assert uncertainty["current_price_validation_status"] == "UNKNOWN"
        assert uncertainty["current_price_observed"] is None
        assert uncertainty["original_price_validation_status"] == "OBSERVED_ONLY"
        assert uncertainty["original_price_observed"] == 68220.0
        assert uncertainty["shop_name_validation_status"] == "UNKNOWN"
        assert uncertainty["discount_percent_validation_status"] == "UNKNOWN"
        assert uncertainty["sold_count_validation_status"] == "UNKNOWN"
        assert uncertainty["rating_validation_status"] == "UNKNOWN"
        assert uncertainty["review_count_validation_status"] == "UNKNOWN"
        assert uncertainty["operation_success_is_not_field_validation_success"] is True
        ...
        assert record["operation_success_is_not_field_validation_success"] is True

        # AC5, AC6, AC7: V5 DOM diagnostic owner and bounded price_role_probe hardening
        assert record["diagnostic_owner"] == "src/product_intelligence/tiktok_pdp_dom_diagnostic.py"
        assert record["diagnostic_artifact_filename"] == V5_DIAGNOSTIC_FILENAME
        assert record["diagnostic_schema_version"] == 5
        assert record["price_role_observability_schema_version"] == 5
        assert record["price_role_observability_hardening_implemented"] is True
        assert record["price_role_resolution_complete"] is False

        # AC8 & AC9: Generation 8 unauthorized, live authorities NONE, selector repair incomplete
        assert record["selector_repair_complete"] is False
        assert record["canonical_evidence_ingested"] is False
        assert record["generation_8_authorized"] is False
        assert record["generation_8_authorized_diagnostic_attempts"] == 0
        assert record["generation_8_authorized_diagnostic_attempts_remaining"] == 0
        assert record["generation_8_diagnostic_execution_owner"] == "NONE"
        assert record["live_dom_diagnostic_authority"] == "NONE"
        assert record["live_public_pdp_acquisition_authority"] == "NONE"
        assert record["automated_public_pdp_acquisition_authority"] == "NONE"
        assert record["market_test_or_action_authority"] == "NONE"
        ...
        # AC9: Planning handoff destination
        assert record["post_publication_handoff"] == (
    ...
    # AC9: Completed TASK-251 revision 1
...
def test_task_252_history_preserves_authorization_without_freezing_current_global_handoff():
    ...
    completed = {item["task_id"]: item for item in state["completed_milestones"]}
    completed_task = completed["TASK-252"]
    document = (
        P8_PUBLIC_PDP_DOM_DIAGNOSTIC_GEN8_AUTHORIZATION_FILE.read_text(
            encoding="utf-8"
        )
    )
    ...
    for record in (authorization, completed_task):
        ...
        assert record["source_task_id"] == "TASK-251"
        assert record["source_run_id"] == TASK_251_RUN_ID
        assert record["source_review_id"] == TASK_251_REVIEW_ID
        assert record["source_published_sha"] == TASK_251_PUBLISHED_SOURCE_SHA
        assert (
        ...
        # AC2: Current uncertainty remains unchanged during authorization engineering
        assert record["current_price_validation_status"] == "UNKNOWN"
        assert record["current_price_observed"] is None
        assert record["original_price_validation_status"] == "OBSERVED_ONLY"
        assert record["original_price_observed"] == 68220.0
        ...
        # AC3: Exact reviewed TASK-252 publication alone grants one fresh Human-owned Gen-8 attempt
        assert record["diagnostic_authorization_generation"] == 8
        assert record["live_dom_diagnostic_authority"] == "ONE_SHOT_ATTACH_ONLY_EXACT_LISTING"
        assert record["generation_8_authorized"] is True
        assert record["generation_8_authorized_diagnostic_attempts"] == 1
        assert record["generation_8_authorized_diagnostic_attempts_remaining"] == 1
        assert record["generation_8_diagnostic_execution_owner"] == "HUMAN_OPERATOR"
        assert record["generation_8_diagnostic_executed"] is False

        # AC4: Published V5 diagnostic/root/sanitization semantics remain frozen with zero production-code modification
        assert record["diagnostic_carrier"] == "src/product_intelligence/tiktok_pdp_dom_diagnostic.py"
        assert record["diagnostic_schema_version"] == 5
        assert record["diagnostic_artifact_filename"] == V5_DIAGNOSTIC_FILENAME
        assert record["diagnostic_artifact_create_exclusive"] is True
        assert record["canonical_cli_command"] == "tiktok-pdp-dom-diagnostic"
        assert record["borrowed_session_count"] == 1
        assert record["session_evaluate_count"] == 1
        assert record["navigation_refresh_click_type_scroll_authority"] == "NONE"
        assert record["browser_lifecycle_authority"] == "TASK-137"
        assert record["task_137_cleanup_preserved"] is True
        assert record["evidence_authority"] == "NONE"
        assert record["price_role_observability_schema_version"] == 5
        assert record["price_role_observability_hardening_implemented"] is True
        assert record["selector_repair_complete"] is False
        ...
        # AC5: Non-consuming preflight proves CDP reachability, 1 normal page, exact PDP, fresh V1-V5-free root, 7-file equivalence
        assert record["preflight_consumes_attempt"] is False
        preflight = record["preflight_authority"]
        assert preflight["cdp_reachability"] == "127.0.0.1:9222"
        assert preflight["normal_type_page_target_total"] == 1
        assert preflight["sole_normal_page_is_exact_selected_pdp"] is True
        assert preflight["selected_pdp_source_id"] == SELECTED_SOURCE_ID
        assert preflight["external_job_root_generation"] == 8
        assert preflight["fresh_external_job_root_requires_v1_absent"] is True
        assert preflight["fresh_external_job_root_requires_v2_absent"] is True
        assert preflight["fresh_external_job_root_requires_v3_absent"] is True
        assert preflight["fresh_external_job_root_requires_v4_absent"] is True
        assert preflight["fresh_external_job_root_requires_v5_absent"] is True
        assert (
            preflight["execution_critical_file_equivalence_source_sha"]
            == TASK_251_PUBLISHED_SOURCE_SHA
        )
        assert preflight["execution_critical_files"] == [
            "src/product_intelligence/tiktok_pdp_dom_diagnostic.py",
        ...
        # AC6: Human invocation is one physical PowerShell line; terminal outcome consumes attempt
        assert (
            record["human_invocation_transport"]
            == "ONE_PHYSICAL_POWERSHELL_LINE_NO_CONTINUATION"
        )
        assert (
            record["pre_carrier_shell_parser_transport_failure_consumes_attempt"]
            is False
        )
        assert record["diagnostic_invocation_consumes_attempt"] is True
        assert (
            record["valid_carrier_invocation_terminal_outcome_consumes_attempt"]
            is True
        )
        assert record["terminal_outcome_consumption"] == "EVERY_TERMINAL_OUTCOME"
        assert record["automatic_retry_refresh_resume"] is False
        assert record["second_invocation_authority"] == "NONE"
        assert record["arbitrary_target_authority"] == "NONE"
        assert record["replacement_target_authority"] == "NONE"
        assert record["search_authority"] == "NONE"
        assert record["batch_authority"] == "NONE"
        assert record["inferred_identity_authority"] == "NONE"
        assert record["variant_switching_authority"] == "NONE"
        assert record["selector_repair_authority"] == "NONE"
        assert record["acquisition_authority"] == "NONE"

        # AC7: Post-attempt Brain review limits, marginal-count semantics, 8 samples, INSUFFICIENT_DIAGNOSTIC_RESOLUTION
        assert record["root_probe_interpretation"] == "ENGINEERING_DIAGNOSTIC_HINT_ONLY"
        assert record["commerce_probe_interpretation"] == "ENGINEERING_DIAGNOSTIC_HINT_ONLY"
        assert record["title_local_topology_probe_interpretation"] == "ENGINEERING_DIAGNOSTIC_HINT_ONLY"
        assert record["price_role_probe_interpretation"] == "ENGINEERING_DIAGNOSTIC_HINT_ONLY"
        assert record["marginal_counts_are_not_intersection_proofs"] is True
        assert record["candidate_samples_capped_at_eight"] is True
        assert record["candidate_samples_not_exhaustive_when_count_exceeds_cap"] is True
        assert record["negative_sample_observations_are_non_exhaustive_when_count_exceeds_cap"] is True
        assert record["truncated_scan_negative_observations"] == "NON_EXHAUSTIVE"
        assert record["range_and_multi_numeric_hints_are_descriptive_only"] is True
        assert record["insufficient_diagnostic_resolution_disposition_allowed"] is True
        assert record["shadow_or_iframe_traversal_authority"] == "NONE"
        assert record["readiness_or_selector_hint_repair_authority"] == "NONE"
        assert (
            record["post_attempt_review_authority"]
            == "HUMAN_BRAIN_GENERATION_8_BOUNDED_PRICE_ROLE_DIAGNOSTIC_REVIEW"
        )
        assert (
            record["mandatory_post_diagnostic_review"]
            == "HUMAN_BRAIN_GENERATION_8_BOUNDED_PRICE_ROLE_DIAGNOSTIC_REVIEW"
        )
        assert (
            record["post_publication_handoff"]
    ...
    # Document contents validation
    assert "C:\\" not in document
    assert document.count("```powershell") == 1
    assert (
        "```powershell\n"
        "python -m src.product_intelligence.cli tiktok-pdp-dom-diagnostic "
        "--job-root <fresh-external-generation-8-job-root> "
...
def test_task_243_authorization_remains_historical_without_freezing_current_handoff():
    ...
    assert completed_task["generation_4_authorized_diagnostic_attempts"] == 1
    assert completed_task["generation_4_authorized_diagnostic_attempts_remaining"] == 0
    assert completed_task["generation_4_diagnostic_execution_owner"] == "HUMAN_OPERATOR"
    assert completed_task["generation_4_diagnostic_executed"] is True
    assert completed_task["diagnostic_outcome"] == "FAIL_CLOSED"
    assert completed_task["diagnostic_failure_reason"] == (
        "BOUNDED_CURRENT_PAGE_EVALUATION_FAILED"
    )
    assert completed_task["diagnostic_process_exit_code"] == 1
    assert completed_task["diagnostic_artifact_created"] is False
    assert completed_task["generation_4_root_probe_observed"] is False
    assert completed_task["generation_4_commerce_probe_observed"] is False
    ...
    assert completed_task["diagnostic_schema_version"] == 2
    assert completed_task["diagnostic_artifact_filename"] == (
        "tiktok-pdp-dom-diagnostic-v2.json"
    )
    assert completed_task["diagnostic_artifact_create_exclusive"] is True
    ...
    assert authorization["session_evaluate_count"] == 1
    assert authorization["navigation_refresh_click_type_scroll_authority"] == "NONE"
    assert completed_task["borrowed_session_count"] == 1
    assert completed_task["session_evaluate_count"] == 1
    assert completed_task["navigation_or_interaction_authority"] == "NONE"
    ...
    assert authorization["task_137_cleanup_preserved"] is True
    assert authorization["human_invocation_transport"] == (
        "ONE_PHYSICAL_POWERSHELL_LINE_NO_CONTINUATION"
    )
    assert authorization["pre_carrier_shell_parser_transport_failure_consumes_attempt"] is False
    assert authorization["diagnostic_invocation_consumes_attempt"] is True
    assert authorization["valid_carrier_invocation_terminal_outcome_consumes_attempt"] is True
    assert authorization["terminal_outcome_consumption"] == "EVERY_TERMINAL_OUTCOME"
    assert authorization["automatic_retry_refresh_resume"] is False
    assert authorization["second_invocation_authority"] == "NONE"
    for authority in (
    ...
    assert authorization["truncated_scan_negative_observations"] == "NON_EXHAUSTIVE"
    assert authorization["shadow_or_iframe_traversal_authority"] == "NONE"
    assert authorization["document_complete_proves_spa_or_network_completion"] is False
    assert authorization["readiness_or_selector_hint_repair_authority"] == "NONE"
    assert handoff["destination"] != authorization["post_publication_handoff"]
