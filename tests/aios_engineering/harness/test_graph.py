...
def _make_repository(tmp_path: Path, files: dict[str, bytes]) -> Path:
    ...
    _git(repository, "config", "user.name", "H4 Tests")
    _git(repository, "config", "user.email", "h4-tests@example.invalid")
...
def test_exact_commit_tree_binding_is_reverified_before_body_read(tmp_path: Path):
    ...
    ranking, _ = rank_repository_evidence(
        discovery,
        TaskRelevanceSpec(task_id="TASK-076", preferred_kinds=(EvidenceKind.SOURCE,)),
    )
...
def test_per_file_edge_bound_fails_without_silent_truncation(tmp_path: Path):
    source = "".join(
        ...
        for index in range(MAX_H4_IMPORT_EDGES_PER_FILE + 1)
    ).encode("utf-8")
...
def test_total_edge_bound_fails_without_silent_truncation(tmp_path: Path):
    file_count = MAX_H4_TOTAL_IMPORT_EDGES // MAX_H4_IMPORT_EDGES_PER_FILE + 1
    files = {
        f"src/source_{file_index}.py": "".join(
            f"import external_{file_index}_{edge_index}\n"
            for edge_index in range(MAX_H4_IMPORT_EDGES_PER_FILE)
        ).encode("utf-8")
...
def test_total_body_bound_is_recomputed_in_selected_order(
    ...
    monkeypatch.setattr(graph_module, "MAX_H4_TOTAL_BODY_BYTES", parsed_size - 1)
...
def test_fingerprints_bind_h2_h3_snapshot_edges_and_receipt_is_zero_authority(
    ...
    assert result.policy_version == H4_GRAPH_POLICY_VERSION
    ...
    assert receipt.generator_version == H4_GRAPH_POLICY_VERSION
