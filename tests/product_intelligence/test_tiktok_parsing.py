...
def test_build_tiktok_candidate_id_determinism_and_fallback() -> None:
    url = "https://www.tiktok.com/view/product/1729482910481234567?track=123#section1"

    # When item ID is known
    id1 = build_tiktok_candidate_id("1729482910481234567", url)
    id2 = build_tiktok_candidate_id("1729482910481234567", url)
    assert id1 == "tiktok_1729482910481234567"
    assert id1 == id2

    # When item ID is unknown, derive cryptographic SHA-256 fingerprint without fragment
    id_fp1 = build_tiktok_candidate_id(None, url)
    id_fp2 = build_tiktok_candidate_id(None, url)
    assert id_fp1.startswith("tiktok_url_")
    assert id_fp1 == id_fp2
    # Ensure fragments/query diffs on same base url are stripped cleanly
    url_diff_params = "https://www.tiktok.com/view/product/1729482910481234567?other=456"
    assert id_fp1 == build_tiktok_candidate_id(None, url_diff_params)
...
def test_build_tiktok_search_url() -> None:
    ...
    # Boundary failures on invalid inputs
...
def test_parsing_has_no_side_effects() -> None:
    # Ensure module is purely functional and imports cleanly without heavy side-effects
    import inspect
